...
#if defined(NDEBUG)
# error "Neexstar cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Neexstar: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Neexstar Signed Message:\n";
...
{
    // Neexstar: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Neexstar: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Neexstar: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc2, 0xb4, 0xde }; // Neexstar: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NeexstarMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NeexstarMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NeexstarMiner(CWallet *pwallet)
{
    printf("NeexstarMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("neexstar-miner");
    ...
    try { loop {
        ...
        printf("Running NeexstarMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NeexstarMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NeexstarMiner, pwallet));
}
